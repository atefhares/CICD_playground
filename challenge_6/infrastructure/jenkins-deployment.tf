    ...
    template {
      ...
      spec {
        ...
        init_container {
          ...
          command = ["bash", "-c", "apt update && apt install -y curl && curl -LO https://storage.googleapis.com/kubernetes-release/release/v1.19.0/bin/linux/amd64/kubectl && chmod +x ./kubectl && mv ./kubectl /usr/local/bin/kubectl"]
          volume_mount {
            name       = "kubectl"
            mount_path = "/usr/local/bin/kubectl"
          }
